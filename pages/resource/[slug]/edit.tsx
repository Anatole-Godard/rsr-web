...
import { CheckIcon, ChevronLeftIcon, CloudUploadIcon, XCircleIcon, XIcon, } from "@heroicons/react/solid";
...
const ResourceEdit: NextPage<any> = (props: Resource) => {
    ...
    const handleSubmit = async (e: { preventDefault: () => void }) => {
        ...
        if (validForm) {
            setLoading(true);
            try {
             const response = await fetchRSR(`/api/resource/${props.slug}/edit`, user?.session, {
               method: "PUT",
    ...
    return (
        <AppLayout>
            <form
                onSubmit={handleSubmit} className="flex flex-col w-full max-h-full bg-white dark:bg-gray-900 grow">
