...
const handler = async(req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (!slug) {
    res.status(400).json({data:null,error: {message:"Missing slug", name: "BadRequestError", code: 400}});
    return;
  }

  const user = await getUser(req)
  if (!user) {
    res.status(401).json({data:null,error: {message:"Unauthorized", name: "UnauthorizedError", code: 401}});
    return;
  }

  const resource = await Resource.findOne({slug});
  if (!resource) {
    res.status(404).json({data:null,error: {message:"Resource not found", name: "ResourceNotFound", code: 404}});
    return;
  }
  ...
  if (seen) {
    ...
    res.status(400).json({data:null,error: {message:"Already seen", name: "AlreadySeenError", code: 400}});
    return;
  } else {
    console.log("Seen");
