...
import { CheckIcon, ChevronDownIcon, SelectorIcon, UserGroupIcon } from "@heroicons/react/solid";
...
const Home: NextPage<any> = ({ resources = [], users = [] }: { resources: Resource[], users: User[] }) => {
  ...
  const [totalUsers, setTotalUsers] = useState<User[]>(users);
  ...
  const getUsersByPeriod = (min, max) => {
    const filtered = users.filter((user) => {
      return (
        moment(min).year() <= moment(user.createdAt).year() &&
        moment(user.createdAt).year() <= moment(max).year() &&
        moment(min).month() <= moment(user.createdAt).month() &&
        moment(user.createdAt).month() <= moment(max).month()
      );
    });
    setTotalUsers(filtered);
  };
  ...
  const exportJSONStatistics = () => {
    ...
    const exportFileDefaultName = "resourcesStatistics.json";
          ...
          <div className='px-5 py-5 mb-2 bg-white rounded-lg dark:bg-gray-800'>
            ...
            <div className='inline-flex w-full mt-2 space-x-2'>
              <Popover className='relative'>
                ...
                <Popover.Panel
                  className='absolute z-10 w-screen max-w-sm mt-3 transform bg-white rounded-lg sm:px-0 lg:max-w-lg dark:bg-black'>
                  {({ close }) => (
                    ...
                    >
                      <Combobox.Options
                        ...
                        {types.length === 0 && query !== "" ? (
                          <div
                            className='relative px-4 py-2 text-gray-700 cursor-default select-none dark:text-gray-300'>
                            Nothing found.
                          </div>
                        ) : (
                          types.map((type) => (
              ...
              <Menu as='div' className='relative'>
                ...
                >
                  <Menu.Items
                    className='absolute right-0 w-56 mt-2 origin-top-right bg-white divide-y divide-gray-100 rounded-md shadow-lg dark:divide-gray-800 dark:bg-black ring-1 ring-black ring-opacity-5 focus:outline-none'>
            ...
            <div className='flex flex-col gap-3 row-span-2'>
              <div className='px-5 py-5 bg-white rounded-lg dark:bg-gray-800'>
                <div className='flex flex-row items-end justify-between'>
                  ...
                  />
                  <LibraryIcon className='w-12 h-12 mr-3 mb-2 shrink-0' />
                </div>
              </div>
              <div className='px-5 py-5 bg-white rounded-lg dark:bg-gray-800'>
                <div className='flex flex-row items-end justify-between'>
                  <GlobalDisplay
                    label='Nombre de vues sur les ressources'
                    data={resourcesFiltered.reduce(((acc, cur) => acc + cur.seenBy.length || 0), 0)}
                  />
                  <EyeIcon className='w-12 h-12 mr-3 mb-2 shrink-0' />
                </div>
              </div>
            </div>
            <div className='px-5 py-5 bg-white rounded-lg dark:bg-gray-800'>
              <div className='flex flex-row items-end justify-between'>
                ...
                />
                <UserGroupIcon className='w-12 h-12 mr-3 mb-2 shrink-0' />
              </div>
            </div>
          </div>
        </div>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  try {
    ...
    const resources: Response = await fetchRSR(
      `http://localhost:3000/api/resource/admin?limit=0`,
      parseUser?.session
    );

    const users: Response = await fetchRSR(
      `http://localhost:3000/api/user/admin?limit=0`,
      parseUser?.session
    );
    ...
    return {
      props: {
        parseUser,
        resources: resourcesBody.data.attributes,
        users: usersBody.data.attributes,
